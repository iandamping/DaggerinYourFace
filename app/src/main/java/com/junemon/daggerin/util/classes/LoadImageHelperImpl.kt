...
class LoadImageHelperImpl @Inject constructor() :LoadImageHelper {
    ...
    override fun ImageView.loadWithGlideSmall(url: String?) {
        Glide.with(this.context).load(url).apply(options.override(150, 150)).placeholder(R.drawable.no_data).thumbnail(0.25f)
            .into(this)
    }

    override fun ImageView.loadWithGlideCustomSize(url: String?, width: Int, height: Int) {
        Glide.with(this.context).load(url).apply(options.override(width, height)).placeholder(R.drawable.no_data).thumbnail(0.25f)
            .into(this)
    }

    override fun ImageView.loadWithGlide(url: String?) {
        Glide.with(this.context).load(url).apply(options).placeholder(R.drawable.no_data).thumbnail(0.25f).into(this)
    }
